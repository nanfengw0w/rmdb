...
### 14. 2026-07-10：修复 load_csv 成功后重启数据丢失
- **现象**: 九张表在 server 运行期间的行数正确，但 load 完成后停止并重启 server，表头中的页数仍是旧值，查询结果为空，评测因此在 load_data 阶段 WA。
- **根因**: `accbfa3` 将 load 命令切换到 `SmManager::load_csv()` 后，删除了旧 direct loader 的收尾刷盘逻辑。记录页、表文件头、索引页和索引文件头只停留在 Buffer Pool 中，没有在 load 成功响应前持久化。
- **修复**: `load_csv()` 在无事务的系统级 load 成功后写回表文件头，刷新表脏页，并刷新每个索引的文件头和脏页；事务上下文不走该路径，避免把未提交数据提前刷盘。
- **验证**: 完整九表加载、八个索引创建、九表行数校验，以及停止/重启后的九表行数校验全部通过；重启后数量为 warehouse=1、district=10、customer=30000、history=300、new_orders=9000、orders=30000、order_line=150000、item=100000、stock=100000。
