...
static bool is_csv_header_tokens_local(const TabMeta &tab, const std::vector<std::string> &tokens) {
    if (tokens.size() != tab.cols.size()) {
        return false;
    }
    for (size_t i = 0; i < tab.cols.size(); ++i) {
        if (lower_local(strip_csv_quotes_local(tokens[i])) != lower_local(tab.cols[i].name)) {
            return false;
        }
    }
    return true;
...
static size_t execute_load_file_direct(const std::string &file_name, const std::string &tab_name) {
    ...
    while (std::getline(input, line)) {
        ...
        if (first_data_line) {
            ...
            if (is_csv_header_tokens_local(tab, tokens)) {
                continue;
            }
        }
