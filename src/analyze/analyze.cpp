...
/**
 * @description: åæå¨ï¼è¿è¡è¯­ä¹åæåæ¥è¯¢éåï¼éè¦æ£æ¥ä¸ç¬¦åè¯­ä¹è§å®çé¨å?
 * @param {shared_ptr<ast::TreeNode>} parse parserçæçç»æé
...
std::shared_ptr<Query> Analyze::do_analyze(std::shared_ptr<ast::TreeNode> parse)
{
    ...
    if (auto x = std::dynamic_pointer_cast<ast::SelectStmt>(parse))
    {
        ...
        // å¤çtarget listï¼åtarget listä¸­æ·»å ä¸è¡¨åï¼ä¾å¦?a.id
        ...
        if (query->cols.empty()) {
            ...
            for (auto &sel_col : query->cols) {
                sel_col = check_column(all_cols, sel_col);  // ååæ°æ®æ ¡éª
            }
        }
        //å¤çwhereæ¡ä»¶
